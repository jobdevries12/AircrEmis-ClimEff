...
df["n"] = df["plev"] / (R * df["tm1"])

# ozone mixing ratio to concentration (molecules/cm³)
df["C_O3"] = df["O3"] * df["n"] * N_A / 1e6  # Convert to cm³
...
pressures = np.sort(df_mean_z["plev"].unique())
...
axes[0].invert_yaxis()
#axes[0].set_yticks([1e5, 1e4, 1e3, 1e2, 1e1, 1])  # Explicitly set tick locations
#axes[0].set_yticklabels(["$10^5$", "$10^4$", "$10^3$", "$10^2$", "$10^1$", "$10^0$"])  # Proper scientific notation
...
axes[1].invert_yaxis()
#axes[1].set_yticks([1e5, 1e4, 1e3, 1e2, 1e1, 1])  # Explicitly set tick locations
#axes[1].set_yticklabels(["$10^5$", "$10^4$", "$10^3$", "$10^2$", "$10^1$", "$10^0$"])  # Proper scientific notation
